\chapter{Méthodes linéaires}
	...
	On a aussi le carré de l'erreur sur tous les objets de $LS$ (Total Square Error), avec $A' = (\ab'(o_1), \dots , \ab'(o_N))$ et $\yb = (y(o_1), y(o_2), \dots , y(o_N))$ :
	...
	1 & 1 & \dots & 1 \\ 
	a_1(o_1) & a_1(o_2) & \dots & a_1(o_N) \\ 
	\vdots & \dots & \dots & \vdots \\ 
	a_n(o_1) & a_n(o_2) & \dots & a_n(o_N)
	\end{pmatrix}}_{N}\right\} n + 1\end{array} $$
	
	\remarque{Ici devrait une étape qui permet de transformer $A$ en une matrice de covariance (voir transparents 10/19 $\rightarrow$ 15/19 sur la régression linéaire.}
	...
	Le problème est que la matrice $A$ est mal conditionnée, notamment elle peut conduire à plusieurs solutions. Pour mieux conditionner le problème, on va introduire un terme $\lambda > 0$ de régulation : l'erreur régulée vaut
...
\begin{eqnarray*}
\frac{\vartheta \mathcal{L}}{\vartheta w} = 0 & \Leftrightarrow & w = \sum_{j = 1}^N \alpha_j \phi(x_j) \\
\frac{\vartheta \mathcal{L}}{\vartheta b} = 0 & \Leftrightarrow & \sum_{k = 1}^N \alpha_k = 0 \\
\frac{\vartheta \mathcal{L}}{\vartheta e_k} = 0 & \Leftrightarrow & \alpha_k = \gamma e_k , \: k = 1, \dots , N\\
\frac{\vartheta \mathcal{L}}{\vartheta \alpha_k} = 0 & \Leftrightarrow &  w^T \phi(x_k) + b + e_k - y_k = 0, \: k = 1, \dots , N
\end{eqnarray*}
